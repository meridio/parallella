...
int main(int argc, char *argv[])
{
	...
	while(1) {
		...
		for(n=0; n < nsteps; n++) {
			...
			// calculate ZA and ZB
			offset=(me.corenum*_MCORE)*sizeof(float);
			if ((me.corenum == 0 ) && (me.myrank==0)) {
			//if (me.corenum == 0 ) {						
				unit_vector_k(dd);
				solve_tridiagonal(aa,bb,cc,dd,_MCORE); //T1ZA=ek
				e_dma_copy((void *)&(Mailbox->tza)+offset, (void*)&dd[0], _size_MCORE);

			} else if ( (me.corenum == (_P/_NBOARDS)-1) && me.islastrank ) {
			//} else if (me.corenum == (_P/_NBOARDS)-1 ) {							
				unit_vector_1(dd);
		...
		// if (me.master) Mailbox->myrank = nsteps;
...
void init()
{
	...
	// get rank info
	me.myrank=Mailbox->myrank;
	me.islastrank=Mailbox->islastrank;
